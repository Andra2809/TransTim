...
class CommonHelper {
  ...
  static Future<Uint8List?> getBytesFromAsset({required String path}) async {
    try {
  ...
  static bool checkIfUrl({required String? string}) {
    try {
      ...
      if (uri?.hasAbsolutePath == true) {
        ...
        return urlMatches.isNotEmpty && uri?.hasScheme == true ? true : false;
      }
    } catch (e) {
      return false;
    }
  ...
  static confirmationDialog({
    ...
    Get.dialog(
      CommonDialog(
        title: capitalizeFirstLetter(title ?? confirmationText),
